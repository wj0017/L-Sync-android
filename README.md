...
개발 기간 2026.05 ~ 2026.09 · 1인 개발 · 개인 저장소에서 개발한 뒤 공개용으로 옮겼다(커밋 이력 유지).
...
| **성경** | 한글·영문 병렬 뷰어, 1년 통독 계획, 검색, 절 묵상 메모 (본문 데이터는 이 저장소에 포함하지 않음 — 아래 참고) |
...
| **알림 설정** | 통독 리마인더·소비 요약(매일/주간) 개별 on/off·시각·요일 지정(홈 메뉴 진입) |
...
- **정기 리마인더** — 지정한 시각에 오늘 통독 잔여 분량과 소비 현황(오늘·이번 달 순지출, 예산 대비, 최근 7일 증감)을 알린다. 오늘 통독을 다 읽은 날에는 보내지 않는다. 기본값은 전부 off.
- **알림·위젯 딥링크** — 알림·위젯을 탭하면 해당 탭으로 이동(콜드스타트 보존). Todo 알림에서 바로 완료 처리.
- **결제 알림 자동 파싱** — 알림 리스너가 결제 알림 제목을 파싱하여 가계부에 자동 입력.
- **성경 데이터 완전 오프라인** — 한글·영문 역본을 Room `createFromAsset` SQLite 번들로 탑재.

> **성경 본문 데이터** — 라이선스 문제로 이 저장소에 포함하지 않는다. `app/src/main/assets/bible.db`·`esv.db`는 스키마만 있는 빈 DB이며, 빌드와 실행은 정상이고 성경 탭은 본문 없이 표시된다. 사용 허가를 받은 역본 데이터를 같은 스키마로 채우면 그대로 동작한다(변환 스크립트: `scripts/`).
...
├── notification/         # AlarmScheduler, ReminderScheduler(정기 리마인더), 결제 알림 파서
...
Firebase 설정 파일은 저장소에 포함하지 않는다(`.gitignore`).

1. [Firebase 콘솔](https://console.firebase.google.com)에서 프로젝트를 만들고 Android 앱(`com.lsync.app`)을 등록한다.
2. Authentication(Google 로그인)과 Firestore를 활성화한다.
3. 내려받은 `google-services.json`을 `app/`에 넣는다.
...
- [x] **Phase 22** — 정기 리마인더 (통독 리마인더·소비 요약 매일/주간, 알림 설정 화면, 발화 시 재무장 + 앱 시작·재부팅·업데이트 복원)
