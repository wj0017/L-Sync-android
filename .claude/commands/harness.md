...
`/docs/` 하위 문서(PRD, TechSpec, ARCHITECTURE, ADR 등)를 읽고 프로젝트의 기획·아키텍처·설계 의도를 파악한다. 필요시 Explore 에이전트를 병렬로 사용한다.
...
5. **AC는 실행 가능한 커맨드** — "~가 동작해야 한다" 같은 추상적 서술이 아닌 `./gradlew assembleDebug` 같은 실제 실행 가능한 검증 커맨드를 포함한다.
6. **주의사항은 구체적으로** — "조심해라" 대신 "X를 하지 마라. 이유: Y" 형식으로 적는다.
7. **네이밍** — step name은 kebab-case slug로, 해당 step의 핵심 모듈/작업을 한두 단어로 표현한다 (예: `room-entity`, `repository-layer`, `compose-screen`).
...
```json
{
  ...
  "phase": "<task-name>",
  "steps": [
    { "step": 0, "name": "room-entity", "status": "pending" },
    { "step": 1, "name": "repository-layer", "status": "pending" },
    { "step": 2, "name": "compose-screen", "status": "pending" }
  ]
}
```
...
반드시 프로젝트 루트(`C:\GitHub\L-Sync`)에서 실행한다.

```bash
python scripts/execute.py {task-name}        # 순차 실행
python scripts/execute.py {task-name} --push  # 실행 후 push (원격 브랜치 생성)
```
