...
class ReminderReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        // 주입은 @AndroidEntryPoint가 생성한 부모가 처리한다. 부모의 onReceive는 abstract이므로
        // super.onReceive를 호출하면 컴파일 에러("Abstract member cannot be accessed directly").
        // TodoActionReceiver와 동일한 패턴이다.
    ...
    private suspend fun buildDailySpendingContent(): Content? {
        ...
        val todayExpense = financeTotals(todayRows).expense

        val ym = todayStr.take(7)
        val monthRows = financeDao.getAllByDateRange(userId, "$ym-01", "$ym-31")
        val monthExpense = financeTotals(monthRows).expense
    ...
    private suspend fun buildWeeklySpendingContent(): Content? {
        ...
        val recentExpense = financeTotals(recent).expense
        val previousExpense = financeTotals(previous).expense
        ...
        val monthExpense = financeTotals(financeDao.getAllByDateRange(userId, "$ym-01", "$ym-31")).expense
    ...
    // 공식을 여기서 다시 쓰지 않고 data/report/MonthlyAggregate.kt(집계 단일 출처)에 위임한다.
    private fun topCategory(rows: List<FinanceEntity>): Pair<String, Long>? =
        topExpenseCategories(rows, limit = 1).firstOrNull()
            ?.takeIf { it.amount > 0 }
            ?.let { it.category to it.amount }
