...
- **소비 집계:** `data/report/MonthlyAggregate.kt`(집계 단일 출처)의 `financeTotals`·`topExpenseCategories`만 호출한다 — 리시버 안에서 정산 공식을 다시 쓰지 않는다(Phase 20 정책). 주간은 **최근 7일 vs 직전 7일** — 사용자가 어떤 요일을 골라도 구간 길이가 같아 비교가 성립한다(월~오늘로 잡으면 수요일 설정 시 3일치만 집계됨).
