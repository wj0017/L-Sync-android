...
## 예산 진행바 (가계부 대시보드)
- 카테고리별·전체 월 한도를 가로 진행바로 표시. `사용액 / 한도`(원) + 백분율.
- **정상:** `AccentBlue` 막대. **초과 시에만** `AccentRed`로 전환(평상시 빨간색 금지 = 절제된 시각 표현).
- 한도 미설정 카테고리는 진행바를 노출하지 않음. 편집 시트에서 전체 한도는 `전체 월 한도`로 표기.

## 통합 검색 화면
- 하단 탭이 아닌 별도 라우트(`search`). 홈 헤더의 검색 아이콘(38dp circle)으로 진입.
- 상단 검색바: 진입 시 **자동 포커스**, 우측 clear(×) 버튼. `BgCard` + HairlineWhite 테두리.
